...
public class FilterCategory implements IRecipeCategory<FilterEntry>
{
	...
	{
		this.id = id;
		drawable = helper.createDrawable(new ResourceLocation("wolfenchantmentlib:textures/inventory.png"), 4, 4, 169, 122);
	...
	{
		return Component.translatable("enchantmentlib.jei.filter");
	}
	...
	public void draw(FilterEntry recipe, @NotNull IRecipeSlotsView view, @NotNull GuiGraphics guiGraphics, double mouseX, double mouseY)
	{
		...
		for(int i = 0;i<Math.min(2, split.size());i++) {
			...
			guiGraphics.drawString(font, split.get(i), (int) 4F, (int) (25F+(i*font.lineHeight)), 4210752);
		}
	}
